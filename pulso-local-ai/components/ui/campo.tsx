...
const BASE =
  "w-full rounded-xl border border-[var(--negocio-borde)] bg-black/25 px-3.5 py-3 " +
...
export function Campo({ etiqueta, id, error, pista, requerido, children }: PropsEnvoltorio) {
  return (
    <div className="flex flex-col gap-1.5">
      ...
      {children}
      {pista && !error ? (
        <p id={`${id}-pista`} className="text-xs text-[var(--negocio-tenue)]">{pista}</p>
      ) : null}
      {error ? (
        <p id={`${id}-error`} role="alert" className="text-xs text-red-300">{error}</p>
      ) : null}
    </div>
  );
}
...
export const Entrada = forwardRef<HTMLInputElement, PropsInput>(
  function Entrada({ className, error, ...props }, ref) {
    return (
      <input
        ref={ref}
        aria-invalid={error || undefined}
        className={cn(BASE, error && "border-red-400/60", className)}
        {...props}
      />
    );
  },
...
type PropsArea = React.TextareaHTMLAttributes<HTMLTextAreaElement> & { error?: boolean };

export const AreaTexto = forwardRef<HTMLTextAreaElement, PropsArea>(
  function AreaTexto({ className, error, ...props }, ref) {
    return (
      <textarea
        ref={ref}
        rows={3}
        aria-invalid={error || undefined}
        className={cn(BASE, "resize-y", error && "border-red-400/60", className)}
        {...props}
      />
    );
  },
...
export const Seleccion = forwardRef<HTMLSelectElement, PropsSelect>(
  function Seleccion({ className, error, children, ...props }, ref) {
    return (
      <select
        ref={ref}
        aria-invalid={error || undefined}
        className={cn(BASE, "appearance-none", error && "border-red-400/60", className)}
        {...props}
      >
        {children}
      </select>
...
export const Casilla = forwardRef<
  HTMLInputElement,
  React.InputHTMLAttributes<HTMLInputElement> & { children: React.ReactNode; error?: string }
>(function Casilla({ children, error, id, className, ...props }, ref) {
  return (
    <div className="flex flex-col gap-1.5">
      <label htmlFor={id} className="flex cursor-pointer items-start gap-3 text-sm leading-snug">
        <input
          ...
          className={cn(
            "mt-0.5 h-5 w-5 shrink-0 cursor-pointer rounded border-[var(--negocio-borde)]",
            "accent-[var(--negocio-acento)]",
            className,
          )}
          {...props}
        />
 ...
 */
export function Honeypot({ registro }: { registro?: Record<string, unknown> }) {
  return (
