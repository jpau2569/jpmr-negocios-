...
// ============================================================================
//  Landing del negocio — /b/[slug]
// ----------------------------------------------------------------------------
//  El orden NO es decorativo, es comercial: primero lo que la persona sentada
//  quiere ahora (el menú de hoy), después lo que le abre el apetito (los
//  destacados), luego la carta, y solo entonces las acciones que convierten
//  (reservar, grupos, opinión, apuntarse). Pedir el correo antes de haber dado
//  nada de valor es la forma más rápida de que cierren la pestaña.
// ============================================================================
    ...
    <>
      ...
      {recomendados.length > 0 ? (
        <Seccion
          ...
          descripcion="Si vienes por primera vez, empieza por aquí."
        >
          <Tarjeta className="px-5 py-1">
            {recomendados.map((p) => (
              <FichaPlato key={p.id} plato={p} negocio={negocio.name} whatsapp={ajustes.whatsapp} />
            ))}
          </Tarjeta>
        </Seccion>
      ) : null}
      ...
      {paraCompartir.length > 0 ? (
        <Seccion
          ...
          descripcion="Para el centro de la mesa."
        >
          <Tarjeta className="px-5 py-1">
            {paraCompartir.map((p) => (
              <FichaPlato key={p.id} plato={p} negocio={negocio.name} whatsapp={ajustes.whatsapp} />
      ...
      {/* --- Carta --- */}
      {modulos.menu !== false ? (
        <Seccion id="carta" titulo="La carta" descripcion={`${espacio.platos.length} platos en ${categorias.length} apartados.`}>
            ...
            <>
              <div className="flex flex-wrap gap-2">
                {categorias.map((c) => (
                  <Link
                    key={c.id}
                    href={`${base}/carta#categoria-${c.id}`}
                    className="rounded-full border border-[var(--negocio-borde)] px-3.5 py-2 text-sm transition-colors hover:border-[var(--negocio-acento)]"
                  >
                    {c.name}
                  </Link>
                ))}
      ...
      {/* --- Menús especiales --- */}
      {modulos.special_menus !== false && especiales.length > 0 ? (
        <Seccion id="especiales" titulo="Menús especiales" descripcion="Fines de semana, celebraciones y jornadas.">
          <Vista evento="special_menu_view" />
          <div className="grid gap-4 sm:grid-cols-2">
            {especiales.map((m) => (
              <Tarjeta key={m.id} className="p-5">
                <h3 className="font-[family-name:var(--font-display)] text-lg">{m.name}</h3>
                {m.description ? (
                  <p className="mt-1 text-sm leading-snug text-[var(--negocio-tenue)]">{m.description}</p>
                ) : null}
                {m.price_cents !== null ? (
                  <p className="mt-2 font-semibold">{euros(m.price_cents)}</p>
                ) : null}
                {m.starts_on || m.ends_on ? (
                  <p className="mt-1 text-xs text-[var(--negocio-tenue)]">
                    {m.starts_on ? `Del ${fechaLarga(m.starts_on)}` : ""}
                    {m.ends_on ? ` al ${fechaLarga(m.ends_on)}` : ""}
                  </p>
                ) : null}
                {m.conditions ? (
                  <p className="mt-2 text-xs text-[var(--negocio-tenue)]">{m.conditions}</p>
                ) : null}
      ...
      {modulos.reservations ? (
        <Seccion id="reservas" titulo="Reservar mesa" descripcion="Dinos cuándo y cuántos sois y te confirmamos.">
      ...
      {modulos.groups ? (
        <Seccion id="grupos" titulo="¿Celebras algo? Cuéntanos tu plan." descripcion="Comuniones, cumpleaños, comidas de empresa o cenas de amigos.">
          <Tarjeta className="p-5">
            <p className="text-sm leading-relaxed text-[var(--negocio-tenue)]">
      ...
      {/* --- Eventos --- */}
      {modulos.events !== false && espacio.eventos.length > 0 ? (
        <Seccion id="eventos" titulo="Eventos y novedades">
          <div className="grid gap-4 sm:grid-cols-2">
            {espacio.eventos.map((e) => (
              <Tarjeta key={e.id} className="p-5">
                ...
                {e.description ? (
                  <p className="mt-2 text-sm text-[var(--negocio-tenue)]">{e.description}</p>
                ) : null}
      ...
      {/* --- Opiniones --- */}
      {modulos.feedback !== false ? (
        <Seccion id="opiniones" titulo="¿Qué tal ha ido?" descripcion="Nos vale más tu opinión sincera que cinco estrellas de compromiso.">
          <Tarjeta className="p-5">
            <p className="text-sm leading-relaxed text-[var(--negocio-tenue)]">
