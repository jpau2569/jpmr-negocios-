...
import { esquemaFeedback } from "@/lib/schemas/formularios";
import { enviar } from "@/lib/enviar";
import { medir } from "@/lib/analitica";
import { cn } from "@/lib/utils";
const CARAS = ["😖", "🙁", "😐", "🙂", "😄"];
export function FormularioOpinion({ slug, negocio, reviewUrl }) {
    ...
    const botonGoogle = reviewUrl ? (<EnlaceBoton href={reviewUrl} target="_blank" rel="noopener" variante="contorno" tamano="bloque" onClick={() => medir("google_review_click")}>
    ...
    if (enviado) {
        ...
        {enviado.demo ? (<Aviso className="mt-4">
      ...
      <form onSubmit={handleSubmit(alEnviar)} className="relative flex flex-col gap-5" noValidate>
        ...
        <fieldset>
          ...
          <div className="mt-4 flex justify-between gap-2" role="radiogroup" aria-label="Puntuación del 1 al 5">
            ...
            return (<button key={valor} type="button" role="radio" aria-checked={activa} aria-label={`${valor} de 5`} onClick={() => elegirNota(valor)} className={cn("flex-1 rounded-xl border py-3 text-2xl transition-all", activa
          ...
          {errors.rating ? (<p role="alert" className="mt-2 text-xs text-red-300">{errors.rating.message}</p>) : null}
          <input type="hidden" {...register("rating")}/>
        </fieldset>

        {nota !== null ? (<>
            <Campo etiqueta={nota <= 3 ? "¿Qué podemos mejorar?" : "¿Qué es lo que más te ha gustado?"} id="op-comentario" pista="Esto lo lee solo el negocio. No se publica en ningún sitio." error={errors.comment?.message}>
              <AreaTexto id="op-comentario" rows={4} placeholder={nota <= 3 ? "Cuéntanoslo con confianza…" : "Lo que quieras contarnos…"} error={Boolean(errors.comment)} {...register("comment")}/>
            </Campo>
            ...
            {quiereContacto ? (<div className="grid gap-4 sm:grid-cols-2">
        ...
        {botonGoogle ? (<div className="border-t border-[var(--negocio-borde)] pt-5">{botonGoogle}</div>) : null}
      </form>
    </Tarjeta>);
}
