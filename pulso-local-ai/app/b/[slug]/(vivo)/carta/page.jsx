...
export default async function PaginaCarta({ params, searchParams }) {
    ...
    return (<div className="px-4 py-8 sm:px-6">
      ...
      <div className="mx-auto w-full max-w-3xl">
        ...
        {conPlatos.length === 0 ? (<Vacio>La carta todavía no está cargada.</Vacio>) : (<>
            <nav aria-label="Apartados de la carta" className="sticky top-[4.2rem] z-30 -mx-4 mt-5 overflow-x-auto border-y border-[var(--negocio-borde)] bg-[var(--negocio-fondo)]/95 px-4 py-2.5 backdrop-blur-md sm:-mx-6 sm:px-6">
              <ul className="flex gap-2">
                {conPlatos.map(({ categoria }) => (<li key={categoria.id}>
                    <a href={`#categoria-${categoria.id}`} className="inline-block whitespace-nowrap rounded-full border border-[var(--negocio-borde)] px-3.5 py-1.5 text-sm">
                      {categoria.name}
                    </a>
                  </li>))}
              </ul>
            ...
            {espacio.platos.some((p) => p.is_demo) ? (<SinConfirmar className="mt-5" texto="Hay platos y precios marcados como muestra: el negocio los confirma antes de publicar. Pregunta en el local cualquier precio o alérgeno concreto."/>) : null}

            <div className="mt-6 space-y-8">
              {conPlatos.map(({ categoria, platos }) => (<section key={categoria.id} id={`categoria-${categoria.id}`} className="scroll-mt-32">
                  <h2 className="font-[family-name:var(--font-display)] text-2xl">{categoria.name}</h2>
                  {categoria.description ? (<p className="mt-1 text-sm text-[var(--negocio-tenue)]">{categoria.description}</p>) : null}
                  <Tarjeta className="mt-3 px-5 py-1">
                    {platos.map((p) => (<FichaPlato key={p.id} plato={p} negocio={negocio.name} whatsapp={ajustes.whatsapp}/>))}
                  </Tarjeta>
                </section>))}
            </div>
