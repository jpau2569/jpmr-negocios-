...
export interface Metricas {
  ...
  clicsResena: number;
  contactos: number;
  porQr: { etiqueta: string; escaneos: number }[];
  platosMasVistos: { nombre: string; vistas: number }[];
}

const ACENTO = "#d4a03c";
const VINO = "#8e2a33";

function Dato({ valor, texto, grande = false }: { valor: number; texto: string; grande?: boolean }) {
  return (
...
export function Resumen({ metricas, demo }: { metricas: Metricas; demo: boolean }) {
  const embudo = [
            ...
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={embudo} margin={{ top: 8, right: 8, left: -16, bottom: 0 }}>
                ...
                <Tooltip
                  ...
                  contentStyle={{ background: "#17181b", border: "1px solid #2a2d33", borderRadius: 10, fontSize: 12 }}
                  labelStyle={{ color: "#f3efe6" }}
                />
                <Bar dataKey="valor" radius={[6, 6, 0, 0]} name="Personas">
                  {embudo.map((_, i) => (
                    <Cell key={i} fill={i === embudo.length - 1 ? VINO : ACENTO} />
                  ))}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
      ...
      <div className="grid gap-6 sm:grid-cols-2">
        <section>
          ...
          <Tarjeta className="mt-3 divide-y divide-white/8">
            {metricas.porQr.length === 0 ? (
              <p className="p-4 text-sm text-white/50">Todavía no hay escaneos registrados.</p>
            ) : (
              metricas.porQr.map((q) => (
                <div key={q.etiqueta} className="flex items-center justify-between px-4 py-3 text-sm">
        ...
        <section>
          ...
          <Tarjeta className="mt-3 divide-y divide-white/8">
            {metricas.platosMasVistos.length === 0 ? (
              <p className="p-4 text-sm text-white/50">Todavía no hay platos abiertos.</p>
            ) : (
              metricas.platosMasVistos.map((p) => (
                <div key={p.nombre} className="flex items-center justify-between gap-3 px-4 py-3 text-sm">
