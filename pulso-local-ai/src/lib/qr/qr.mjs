// ============================================================================
//  PULSO LOCAL AI — generador de códigos QR sin dependencias
// ----------------------------------------------------------------------------
//  Mismo algoritmo que `fotos-faciles/nucleo/qr.mjs` del monorepo (solo cambia
//  esta cabecera). El test `test/qr.test.mjs` comprueba que los dos módulos
//  siguen generando exactamente la misma matriz.
//  Se reutiliza en vez de instalar una librería de QR por dos motivos: el
//  algoritmo ya está verificado módulo a módulo contra la librería `qrcode`, y
//  un QR impreso en un cartel no puede depender de que un paquete de npm no se
//  rompa en la siguiente versión mayor.
//
//  Implementa QR modelo 2, modo byte (UTF-8), versiones 1-10 y niveles L/M/Q/H:
//  de sobra para las URL cortas que genera el panel (`/q/castresana-demo-001`).
//
//  API:  matriz(texto, { nivel })  → { tamano, modulos }
//        svg(texto, opciones)      → cadena SVG lista para incrustar
//        ascii(texto, opciones)    → QR para pintar en el terminal
// ============================================================================
