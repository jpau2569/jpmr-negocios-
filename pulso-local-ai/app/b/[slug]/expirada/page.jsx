...
export default async function PaginaExpirada({ params }) {
    const { slug } = await params;
    const espacio = await leerEspacio(slug);
    if (!espacio)
        notFound();
    ...
    const comercial = ajustes.reactivation_whatsapp
        ...
        ?? null;
    const wasap = enlaceWhatsapp(comercial, "Pulso Local AI", { tipo: "evento", evento: `reactivar el espacio de ${negocio.name}` });
    return (<>
