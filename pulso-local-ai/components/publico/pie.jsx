...
const REDES = [
    ...
    { clave: "tripadvisor", nombre: "TripAdvisor" },
];
export function Pie({ espacio }) {
    ...
    const pendientes = ajustes.pending_notes ?? [];
    const redes = REDES
        .map((r) => ({ ...r, url: ajustes[r.clave] }))
        .filter((r) => typeof r.url === "string" && r.url.startsWith("http"));
    return (<footer className="border-t border-[var(--negocio-borde)] px-4 py-10 sm:px-6">
      <div className="mx-auto grid w-full max-w-3xl gap-8 sm:grid-cols-3">
        <div>
          ...
          {ajustes.address ? (<p className="mt-2 text-sm text-[var(--negocio-tenue)]">{ajustes.address}</p>) : null}
        ...
        <div>
          ...
          <ul className="mt-2 space-y-1 text-sm text-[var(--negocio-tenue)]">
            ...
            {ajustes.email ? (<li><a href={`mailto:${ajustes.email}`} className="underline underline-offset-4">{ajustes.email}</a></li>) : null}
      ...
      {(demo.esDemo || sinConfirmar || pendientes.length > 0) && (<div className="mx-auto mt-8 w-full max-w-3xl rounded-xl border border-amber-400/25 bg-amber-400/[0.07] px-4 py-3">
          ...
          {pendientes.length > 0 ? (<ul className="mt-2 space-y-0.5 text-xs text-amber-100/65">
              {pendientes.map((nota) => <li key={nota}>· {nota}</li>)}
            </ul>) : null}
        </div>)}
