...
export function Cabecera({ espacio }: { espacio: EspacioNegocio }) {
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-[var(--negocio-borde)] bg-[var(--negocio-fondo)]/92 backdrop-blur-md">
      <div className="mx-auto flex w-full max-w-3xl items-center gap-3 px-4 py-3 sm:px-6">
        ...
        <div className="min-w-0 flex-1">
          ...
          {apertura ? (
            <p className="truncate text-xs text-[var(--negocio-tenue)]">{apertura}</p>
          ) : ajustes.address ? (
            <p className="truncate text-xs text-[var(--negocio-tenue)]">{ajustes.address}</p>
          ) : null}
        </div>
