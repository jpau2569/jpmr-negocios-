...
import { esquemaGrupo, type DatosGrupo } from "@/lib/schemas/formularios";
...
export const TEXTO_RGPD_GRUPO =
  "Acepto que el negocio use mis datos para preparar una propuesta y contactarme sobre esta celebración.";

export function FormularioGrupo({ slug, negocio, whatsapp }: Props) {
  ...
  async function alEnviar(datos: DatosGrupo) {
    setFallo(null);
      ...
      <Tarjeta className="p-6">
        ...
        {wasap ? (
          <EnlaceBoton href={wasap} target="_blank" rel="noopener" variante="principal"
            tamano="bloque" className="mt-5" onClick={() => medir("whatsapp_click")}>
  ...
  return (
    <Tarjeta className="p-6">
      <form onSubmit={handleSubmit(alEnviar)} className="relative flex flex-col gap-4" noValidate
        onFocusCapture={() => medir("group_request_start")}>
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <Campo etiqueta="Teléfono" id="gr-telefono" requerido error={errors.phone?.message}>
            <Entrada id="gr-telefono" type="tel" inputMode="tel" autoComplete="tel"
              error={Boolean(errors.phone)} {...register("phone")} />
          </Campo>
        </div>

        <div className="grid gap-4 sm:grid-cols-2">
          <Campo etiqueta="¿Qué día?" id="gr-dia" pista="Si aún no lo sabes, déjalo vacío"
          ...
          <Campo etiqueta="¿Cuántos sois?" id="gr-personas" error={errors.party_size?.message}>
            <Entrada id="gr-personas" type="number" inputMode="numeric" min={1} max={500}
              placeholder="Aproximado" {...register("party_size")} />
          </Campo>
        </div>
        ...
        <Campo etiqueta="Presupuesto por persona" id="gr-presupuesto"
          pista="Orientativo y opcional. Nos ayuda a proponerte algo que encaje."
          error={errors.budget_hint?.message}>
        ...
        <Campo etiqueta="Cuéntanos tu plan" id="gr-comentarios" error={errors.comments?.message}>
          <AreaTexto id="gr-comentarios" rows={4}
            placeholder="Somos unos 25, hay dos celíacos y tres niños, nos gustaría zona reservada…"
            {...register("comments")} />
        </Campo>
