...
export const estilosBoton = cva("inline-flex items-center justify-center gap-2 rounded-full font-semibold " +
    "transition-[transform,background-color,opacity] duration-150 " +
    "active:scale-[0.98] disabled:pointer-events-none disabled:opacity-50 " +
    "whitespace-nowrap select-none", {
    variants: {
        variante: {
            principal: "bg-[var(--negocio-acento)] text-[var(--negocio-sobre-acento)] hover:brightness-110",
            acento: "bg-[var(--negocio-acento2)] text-[var(--negocio-sobre-acento2)] hover:brightness-110",
            contorno: "border border-[var(--negocio-borde)] bg-transparent text-[var(--negocio-texto)] hover:bg-white/5",
            plano: "bg-white/5 text-[var(--negocio-texto)] hover:bg-white/10",
            fantasma: "bg-transparent text-[var(--negocio-tenue)] hover:text-[var(--negocio-texto)]",
        },
        tamano: {
...
export function Boton({ className, variante, tamano, ...props }) {
    return <button className={cn(estilosBoton({ variante, tamano }), className)} {...props}/>;
}
export function EnlaceBoton({ className, variante, tamano, ...props }) {
