...
export async function rpc(fn, args) {
  ...
  // Timeout: si Supabase no responde, no dejamos la petición colgada.
  const ctrl = new AbortController();
  const alarma = setTimeout(() => ctrl.abort(), 15000);
  let resp;
  try {
    resp = await fetch(`${process.env.SUPABASE_URL}/rest/v1/rpc/${fn}`, {
      method: "POST",
      signal: ctrl.signal,
      headers: {
        apikey: clave,
    ...
    const err = new Error(
      e?.name === "AbortError" ? "la memoria en la nube tardó demasiado" : `no se pudo conectar con la nube: ${e?.message || e}`
    );
    err.status = 504;
    throw err;
  } finally {
