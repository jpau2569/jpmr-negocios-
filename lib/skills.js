...
//   2. Carpeta skills/<nombre>/SKILL.md — el formato estándar de skills
//      (cabecera con name y description + instrucciones en markdown). Basta
//      con dejar una carpeta nueva ahí para que Clara la tenga.
//   3. Supabase (tabla clara_skills) — skills que Clara crea o mejora ella misma
...
const CARPETA_SKILLS = new URL("../skills/", import.meta.url);

// Lee la cabecera "--- name: … description: … ---" de un SKILL.md.
export function parsearSkillMd(texto) {
  const t = String(texto || "").replace(/^\uFEFF/, "");
  const m = t.match(/^---\s*\n([\s\S]*?)\n---\s*\n?([\s\S]*)$/);
  if (!m) return null;
  const meta = {};
  for (const linea of m[1].split("\n")) {
    const kv = linea.match(/^([A-Za-z_-]+)\s*:\s*(.*)$/);
    if (kv) meta[kv[1].toLowerCase()] = kv[2].trim().replace(/^["']|["']$/g, "");
  }
  const nombre = String(meta.name || "").trim().toLowerCase();
  if (!/^[a-z0-9][a-z0-9-]{1,59}$/.test(nombre) || !meta.description) return null;
  return { nombre, descripcion: meta.description.slice(0, 500), contenido: m[2].trim() };
}

let cacheArchivos = null;
// Skills de la carpeta skills/ (se leen una vez por arranque de la función).
export function skillsDeArchivo(carpeta = CARPETA_SKILLS) {
  if (cacheArchivos && carpeta === CARPETA_SKILLS) return cacheArchivos;
  const out = {};
  let dirs = [];
  try {
    dirs = readdirSync(carpeta, { withFileTypes: true }).filter((d) => d.isDirectory());
  } catch {
    dirs = [];
  }
  for (const d of dirs) {
    try {
      const s = parsearSkillMd(readFileSync(new URL(`${d.name}/SKILL.md`, carpeta), "utf8"));
      if (s && !SKILLS_BASE[s.nombre]) out[s.nombre] = { descripcion: s.descripcion, contenido: s.contenido };
    } catch {
      // carpeta sin SKILL.md: se ignora
    }
  }
  if (carpeta === CARPETA_SKILLS) cacheArchivos = out;
  return out;
}
...
export async function catalogoSkills(claveSync) {
  ...
  const archivos = Object.entries(skillsDeArchivo()).map(([nombre, s]) => ({
    nombre,
    descripcion: s.descripcion,
    origen: "SKILL.md",
  }));
  ...
  return [...base, ...archivos, ...personalizadas];
}
...
export async function leerSkill(claveSync, nombre) {
  ...
  const archivo = skillsDeArchivo()[limpio];
  if (archivo) return archivo.contenido;
  if (claveSync && nubeConfigurada()) {
...
export async function guardarSkill(claveSync, nombre, descripcion, contenido) {
  ...
  if (skillsDeArchivo()[limpio]) {
    throw new Error(`"${limpio}" es una skill de la carpeta skills/ y no se puede sobrescribir; elige otro nombre`);
  }
