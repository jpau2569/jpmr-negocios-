...
import { esquemaLead, type DatosLead } from "@/lib/schemas/formularios";
...
export const TEXTO_CONSENTIMIENTO =
  "Acepto recibir el menú del día, novedades y eventos del negocio por el canal que he indicado. " +
  "Puedo darme de baja en cualquier momento escribiendo al propio negocio. Mis datos no se ceden a terceros.";

export function BloqueFidelizacion({ espacio }: { espacio: EspacioNegocio }) {
  const { negocio } = espacio;
  const [hecho, setHecho] = useState<{ demo: boolean } | null>(null);
  ...
  async function alEnviar(datos: DatosLead) {
    setFallo(null);
  ...
  if (hecho) {
    return (
      <Seccion id="novedades" titulo="Hecho">
        <Aviso tono="ok">
  ...
  return (
    <Seccion
      id="novedades"
      titulo="Recibe el menú, novedades y próximos eventos"
      descripcion="Sin spam. Solo cuando hay algo que contar."
    >
        ...
        <form onSubmit={handleSubmit(alEnviar)} className="relative flex flex-col gap-4" noValidate>
          ...
          <fieldset>
            ...
            <div className="mt-2 flex flex-wrap gap-x-5 gap-y-2">
              {INTERESES.map((i) => (
                <label key={i.valor} className="flex cursor-pointer items-center gap-2 text-sm">
                  <input
                    type="checkbox"
                    value={i.valor}
                    className="h-4.5 w-4.5 accent-[var(--negocio-acento)]"
                    {...register("interests")}
                  />
