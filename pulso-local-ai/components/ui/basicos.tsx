...
import { ETIQUETAS_ES, type EtiquetaPlato } from "@/types/negocio";
...
export function Tarjeta({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) {
  return (
    <div
      className={cn(
        "rounded-[var(--radius-tarjeta)] border border-[var(--negocio-borde)]",
        "bg-[var(--negocio-superficie)] overflow-hidden",
        className,
      )}
      {...props}
    />
...
export function Seccion({
  id, titulo, descripcion, children, className,
}: {
  id: string;
  titulo: string;
  descripcion?: string;
  ...
  return (
    <section id={id} className={cn("scroll-mt-20 px-4 py-10 sm:px-6", className)}>
      <div className="mx-auto w-full max-w-3xl">
        <h2 className="font-[family-name:var(--font-display)] text-2xl sm:text-3xl">{titulo}</h2>
        {descripcion ? (
          <p className="mt-1.5 text-sm text-[var(--negocio-tenue)]">{descripcion}</p>
        ) : null}
...
const COLOR_ETIQUETA: Partial<Record<EtiquetaPlato, string>> = {
...
export function Etiqueta({ nombre }: { nombre: string }) {
  const conocida = nombre in ETIQUETAS_ES ? (nombre as EtiquetaPlato) : null;
  return (
    <span
      className={cn(
        "inline-flex items-center rounded-full px-2 py-0.5 text-[0.7rem] font-semibold",
        conocida ? COLOR_ETIQUETA[conocida] : "bg-white/10 text-[var(--negocio-texto)]",
      )}
    >
...
export function SinConfirmar({ texto, className }: { texto?: string; className?: string }) {
  return (
    <p
      className={cn(
        "inline-flex items-start gap-1.5 rounded-lg bg-amber-400/10 px-2.5 py-1.5",
        "text-[0.72rem] leading-snug text-amber-200/90",
        className,
      )}
    >
...
export function Aviso({
  ...
  return (
    <div className={cn("rounded-xl border px-4 py-3 text-sm leading-relaxed", tonos[tono], className)}>
      {children}
    </div>
  );
}
...
export function Vacio({ children }: { children: React.ReactNode }) {
  return (
    <p className="rounded-xl border border-dashed border-[var(--negocio-borde)] px-4 py-8 text-center text-sm text-[var(--negocio-tenue)]">
      {children}
    </p>
  );
}
