...
export default async function Portada() {
    ...
    const demos = await Promise.all(slugsConocidos().map(async (slug) => ({ slug, espacio: await leerEspacio(slug) })));
    return (<div className="pizarra min-h-dvh bg-[#0e0f11] px-5 py-14 text-[#f3efe6]">
      <div className="mx-auto w-full max-w-3xl">
        ...
        <div className="mt-4 grid gap-4 sm:grid-cols-2">
          {demos.map(({ slug, espacio }) => {
            ...
            return (<div key={slug} className="rounded-[14px] border border-white/10 bg-[#17181b] p-5">
                <div className="flex items-start gap-4">
                  <div className="h-20 w-20 shrink-0 rounded-lg bg-white p-1" dangerouslySetInnerHTML={{ __html: qrSvg(enlace, { margen: 1 }) }}/>
                  <div className="min-w-0">
                    <h3 className="font-[family-name:var(--font-display)] text-lg leading-tight">
                      {espacio.negocio.name}
                    </h3>
                    {espacio.ajustes.tagline ? (<p className="mt-1 text-xs leading-snug text-white/55">{espacio.ajustes.tagline}</p>) : null}
