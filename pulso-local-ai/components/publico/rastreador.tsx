...
export function Rastreador({
  ...
  return (
    <span
      className="contents"
      onClickCapture={() => medir(evento, subjectId ? { subject_id: subjectId } : {})}
    >
      {children}
    </span>
  );
...
export function Vista({ evento, subjectId }: { evento: TipoEventoAnalitica; subjectId?: string }) {
 ...
 */
export function IniciarAnalitica({ slug, qr }: { slug: string; qr?: string | null }) {
  useEffect(() => {
    let token = qr ?? null;
    try {
