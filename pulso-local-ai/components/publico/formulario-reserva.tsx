...
import { esquemaReserva, type DatosReserva } from "@/lib/schemas/formularios";
...
export const TEXTO_RGPD_RESERVA =
  "Acepto que el negocio use mi nombre y teléfono para gestionar esta reserva y contactarme sobre ella.";

export function FormularioReserva({ slug, negocio, telefono, whatsapp }: Props) {
  ...
  async function alEnviar(datos: DatosReserva) {
    setFallo(null);
  ...
  return (
    <Tarjeta className="p-6">
      <form onSubmit={handleSubmit(alEnviar)} className="relative flex flex-col gap-4" noValidate
        onFocusCapture={() => medir("reservation_start")}>
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <Campo etiqueta="Teléfono" id="rv-telefono" requerido error={errors.phone?.message}
            pista="Para confirmarte la mesa">
            <Entrada id="rv-telefono" type="tel" inputMode="tel" autoComplete="tel"
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          <Campo etiqueta="Día" id="rv-dia" requerido error={errors.service_date?.message}>
            <Entrada id="rv-dia" type="date" min={hoyISO()} max={sumaDias(hoyISO(), 120)}
              error={Boolean(errors.service_date)} {...register("service_date")} />
          </Campo>
          ...
          <Campo etiqueta="Personas" id="rv-personas" requerido error={errors.party_size?.message}>
            <Entrada id="rv-personas" type="number" inputMode="numeric" min={1} max={200}
              error={Boolean(errors.party_size)} {...register("party_size")} />
          </Campo>
        </div>
        ...
        <Campo
          etiqueta="Alergias o intolerancias"
          id="rv-alergias"
          pista="Opcional, para poder prepararlo. Confírmalo también al llegar con el personal."
          error={errors.allergies_note?.message}
        >
