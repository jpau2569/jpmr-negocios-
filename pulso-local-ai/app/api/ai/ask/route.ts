...
import { ALERGENOS_ES, type Alergeno } from "@/types/negocio";
...
const NO_LO_SE =
  "Para confirmarlo, contacta directamente con el local por teléfono o WhatsApp.";

const AVISO_ALERGIAS =
  "Indica siempre la alergia al personal para confirmar opciones y evitar contaminación cruzada.";

interface Respuesta {
  texto: string;
  /** De dónde sale cada cosa: sin fuente, no se afirma. */
  fuentes: { tipo: string; id?: string; nombre: string }[];
  avisoAlergias?: boolean;
}
...
const contiene = (texto: string, palabras: string[]) =>
  palabras.some((p) => sinAcentos(texto).includes(sinAcentos(p)));
...
export async function POST(peticion: Request) {
  ...
  if (!validado.success) return error(validado.error.issues[0]?.message ?? "Pregunta no válida.");
  ...
  if (!espacio) return error("Este espacio no está disponible.", 404);
...
function responder(
  pregunta: string,
  espacio: Awaited<ReturnType<typeof leerEspacio>>,
): Respuesta {
  if (!espacio) return { texto: NO_LO_SE, fuentes: [] };
  ...
  if (contiene(q, ["menu del dia", "menú del día", "hay hoy", "de menu", "que hay de comer"])) {
    ...
    if (menuDeHoy.is_demo) {
      return {
        texto:
          `El menú que aparece en la web es de muestra: ${negocio.name} todavía no ha cargado ` +
          `el de hoy. ${NO_LO_SE}`,
  ...
  const alergenoPreguntado = (Object.keys(ALERGENOS_ES) as Alergeno[])
  ...
  if (alergenoPreguntado || contiene(q, ["alergia", "alergeno", "alérgeno", "celiaco", "celíaco"])) {
    ...
    return {
      texto:
        `Estos platos declaran ${ALERGENOS_ES[alergenoPreguntado].toLowerCase()}: ` +
