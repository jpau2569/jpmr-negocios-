...
// ============================================================================
export function FichaPlato({ plato, negocio, whatsapp, conFoto = true, }) {
    const wasap = enlaceWhatsapp(whatsapp, negocio, { tipo: "plato", plato: plato.name });
    const agotado = plato.status === "sold_out";
    return (<article id={`plato-${plato.id}`} className={`flex gap-3.5 border-b border-[var(--negocio-borde)] py-4 last:border-0 ${agotado ? "opacity-55" : ""}`}>
      {conFoto && plato.image_url ? (
        // eslint-disable-next-line @next/next/no-img-element
        <img src={plato.image_url} alt={plato.name} loading="lazy" className="h-20 w-20 shrink-0 rounded-xl object-cover sm:h-24 sm:w-24"/>) : null}
      ...
      <div className="min-w-0 flex-1">
        ...
        {plato.description ? (<p className="mt-0.5 text-sm leading-snug text-[var(--negocio-tenue)]">{plato.description}</p>) : null}

        {plato.tags.length > 0 || agotado ? (<div className="mt-1.5 flex flex-wrap gap-1.5">
            {agotado ? <Etiqueta nombre="Agotado hoy"/> : null}
            {plato.tags.map((t) => <Etiqueta key={t} nombre={t}/>)}
          </div>) : null}

        {plato.alergenos.length > 0 ? (<p className="mt-1.5 text-[0.7rem] text-[var(--negocio-tenue)]">
            Alérgenos declarados: {plato.alergenos.map((a) => ALERGENOS_ES[a]).join(", ")}
          </p>) : null}

        {plato.is_demo ? (<p className="mt-1.5 text-[0.7rem] text-amber-200/80">
            ⚠ Nombre y precio sin confirmar por el negocio
          </p>) : null}

        {wasap ? (<Rastreador evento="whatsapp_click" subjectId={plato.id}>
            <a href={wasap} target="_blank" rel="noopener" className="mt-2 inline-block text-xs font-medium underline underline-offset-4">
