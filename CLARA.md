...
- **📲 Compartir con Clara (Android)** — `clara.webmanifest` declara `share_target` y `clara-sw.js` (service worker limitado a `/clara`) recibe lo que compartes desde WhatsApp u otra app: hasta **6 fotos o PDFs** más el texto o enlace. El chat los adjunta solos (reduciendo las fotos en el móvil) y prepara el mensaje "📲 Compartido desde WhatsApp… Es del piso: ". **Cómo se usa:** 1) abre Clara en Chrome del móvil → menú ⋮ → *Instalar aplicación* (o *Añadir a pantalla de inicio*) y ábrela una vez; 2) en WhatsApp, mantén pulsadas las fotos o el mensaje del cliente → *Compartir* → **Clara**; 3) escribe de qué piso es y qué quieres (por ejemplo, la ficha para Inmoweb). Clara no accede a tu WhatsApp: solo ve lo que tú le compartes. En iPhone no está disponible (Safari no admite recibir lo compartido); allí se usa el clip 📎, que ahora también admite varias fotos a la vez. Prueba en navegador real: `test/clara-compartir.ui.test.mjs`.
